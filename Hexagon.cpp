...
Hexagon::Hexagon(Point click, Color colorFill) : Polygon(click, colorFill) {
	this->construct();
}
...
void Hexagon::construct() {
    ...
    int minX = clickmouse.getX() + distance * std::cos(0);
    int minY = clickmouse.getY() + distance * std::sin(0);
